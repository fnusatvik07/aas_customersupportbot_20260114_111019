# ui_user_3993a524/main.py
"""
Customer Support Bot - Auto-generated FastAPI service
Role: Automated Customer Service Specialist
"""
import asyncio
import json
import os
...
app = FastAPI(
    ...
    description="Automated Customer Service Specialist",
    version="1.0.0"
)
...
CORS_ORIGINS = [
    ...
    f"http://localhost:8056",
    f"http://localhost:8057", 
    f"http://localhost:8058",
    f"http://127.0.0.1:8056",
    f"http://127.0.0.1:8057",
    f"http://127.0.0.1:8058"
]
...
AGENT_CONFIG = {
    ...
    "role": "Automated Customer Service Specialist",
    "tools": ['AskUserQuestion', 'TodoWrite', 'Read', 'Write', 'WebFetch'],
    "system_prompt": """You are a Customer Support Bot with an analyst personality, designed to efficiently handle customer inquiries and maintain service quality metrics.

Your primary responsibilities:
1. RESPOND TO INQUIRIES: Analyze customer questions and provide clear, accurate, and helpful responses. Use your knowledge base and available resources to resolve common issues.

2. INTELLIGENT ESCALATION: Identify complex issues that require human intervention. Escalate when:
   - Technical problems beyond standard troubleshooting
   - Billing disputes or refund requests
   - Customer expresses high frustration or dissatisfaction
   - Issues requiring policy exceptions or management approval
   - Security or account compromise concerns

3. TRACK METRICS: Systematically log all interactions including:
   - Inquiry timestamp and type
   - Resolution time
   - Escalation status
   - Customer satisfaction indicators
   - Common issue patterns

4. CLARIFICATION: When customer inquiries are unclear, ask targeted questions to understand the issue fully before responding.

5. DOCUMENTATION: Maintain organized records of all customer interactions and resolutions for analysis and continuous improvement.
...
- Data-driven in tracking and reporting

Always prioritize customer satisfaction while maintaining efficient resolution times. Use data to identify trends and improve service quality.""",
...
async def root():
    return {
        "message": "Welcome to Customer Support Bot",
        "role": "Automated Customer Service Specialist",
        "agent_id": "ui_user_3993a524",
        "endpoints": [
...
async def get_frontend_info():
    ...
    return {
        "backend_url": f"http://localhost:8055",
        "frontend_url": f"http://localhost:8056",
        "cors_enabled": True,
...
async def stream_agent_progress(request: QueryRequest) -> AsyncGenerator[str, None]:
    ...
    try:
        ...
        yield create_event("progress", {
            "message": "[AGENT] Starting Customer Support Bot...",
...
async def query_agent(request: QueryRequest):
    """
    Send a query/task to the Customer Support Bot (non-streaming)
    """
    try:
        # Create Claude Agent SDK options
        options = ClaudeAgentOptions(
...
if __name__ == "__main__":
    ...
    print(f"[INFO] Customer Support Bot Backend Starting...")
    print(f"[BACKEND] Backend URL: http://localhost:8055")
    print(f"[FRONTEND] Frontend URL: http://localhost:8056")  
    print(f"[CORS] CORS: Enabled for frontend connections")
    print(f"[DOCS] API Docs: http://localhost:8055/docs")
    print(f"[HEALTH] Health: http://localhost:8055/health")
